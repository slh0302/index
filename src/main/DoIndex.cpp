...
//	std::cout<<argv[0]<<std::endl;
	while ((ch = getopt(argc, argv, "iacod")) != EOF)
	{
		switch (ch)
		{
		case 'i':
...
//	ofstream out_te("/home/slh/test/test/reslut.txt",std::ios::app);
//	out_te<<(int)Array_Data[0];
//	for(int i=1;i<=count*TOTALBYTESIZE-1;i++){
//		if(i%1024==0){
//			out_te<<endl;
//		}else out_te<<" "<<(int)Array_Data[i];
//	}	
...
int calc_feature_pipeline(int argc, char** argv){
   ...
   //  google::SetLogDestination(google::INFO,"/home//test_log/");
	...
	if(!std::fstream(table_filename.c_str())) std::cout<<"11111111111111111"<<std::endl;
...
void feature_extraction_pipeline(char* p,const char* CPU_MODE, int count,int GPU_ID) {
	...
	ofstream out_te("/home/slh/test/test/yu",std::ios::app);
